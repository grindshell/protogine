...
collision code, and its live probe reads engine-resolved positions. M2's contract
is frozen as of 2026-09-08 in the
[M2 contract](docs/implementation/TILEMAP_COLLISION_M2.md) - **its rules settled
after three review rounds, its numbers still awaiting the Phase 0 probe, and no
implementation behind either**; M3 and M4 remain undesigned. All three are required core scope, so the overall feature and this
plan stay open. Acceptance is not execution evidence.
