...
| Aggregate live cells | 524,288 | 1 MiB of cell storage, twice M1's single-map allowance, and it fits inside one callback with the arithmetic stated rather than asserted. A Luau install charges exactly once per cell - `EngineContext::candidate` wires `charge_callback_work` into `solid_flags` and `cell_ids`, and the kernel adds nothing for the install itself, which is why `installing_the_room_costs_exactly_what_its_cells_and_flags_cost` lands on 510 + 5 + 1 = 516 rather than the 1,026 a second charge per cell would add. **The `+ 1` is the attachment, not the install**, and an earlier draft of this row attributed it to `Kernel::set_tilemap` revalidating existing members: at install time that sample has no members, because it installs before it spawns. It is `set_tile_collider`'s `check_placement` charging for the one tile the box covers. The row's point - 516 and not 1,026 - is unaffected, which is exactly why the wrong attribution could sit here unchallenged. Found by the review session. Note also that the charge lives at the *binding* and not in `create_tilemap`, so each new installing binding is a new charge site and the pinned total is what catches both a dropped charge and a doubled one. So filling the whole budget costs 524,288 cells plus at most 64 x 1,024 solid flags = **589,824 of 1,048,576**, leaving 44% spare |
